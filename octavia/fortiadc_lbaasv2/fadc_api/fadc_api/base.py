...
class FADC(object):
    ...
    def get(self, url_postfix, params=None, data=None):
        ...
        try:
            ...
            if res.status_code != 200:
                ...
                LOG.debug("res= %s" % (res))
                LOG.debug( 'GET: res %s cookies %s' % (res, self.session.cookies))
    ...
    def action_handler(self, method, errcodes, obj):
        ...
        elif method in [RequestAction.DELETE, RequestAction.UPDATE, RequestAction.GETONE]:
    ...
    def check_response(self, response):
        ...
        if response.status_code == 200:
            ...
            try:
                ...
                if 'payload' in res:
                    if isinstance (res['payload'], basestring):
            ...
            try:
                ...
                if self.verbose:
                    LOG.debug( 'Fail with status: {0}'.format(response.status_code))
                if 'message' in res:
                    if isinstance (res['message'], basestring):
    ...
    def response_handler(self, response, errcodes):
        if response.status_code == HTTPStatus.FAILED_DEPENDENCY:
            LOG.error(
                ...
                'Will retry add vdom %s.' % self.tenant_id
            )
            return 0
        elif response.status_code == HTTPStatus.FORBIDDEN:
            LOG.error(
                'ERROR: Adding vdom %s is missing CSRF token or administrator '
                'is missing access profile permissions.' % (self.tenant_id)
            )
            return 0
