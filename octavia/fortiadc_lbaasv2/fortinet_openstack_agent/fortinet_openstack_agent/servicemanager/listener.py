...
from fadc_api.slb.virtual_server import VirtualServer
...
class Listener(object):
        ...
        if not _pool:
            ...
            try:
                ...
                if listener.fadc_pktfwd != '' and listener.fadc_nat_pool and listener.protocol == 'TCP':
                    ...
                    nat_pool.create(listener.fadc_nat_pool, listener.fadc_vs_nat_intf, listener.tenant_id)
    ...
    def get_stats(self, listeners):
        return self.vs.get_all_vs_stats(listeners, self.conf.fadc_get_stats_interval)

    def check_parameter(self, listener):

        listener.fadc_pktfwd = self.conf.fadc_vs_packet_forward_method
        listener.fadc_nat_pool = self.conf.fadc_vs_nat_pool
        listener.fadc_vs_nat_intf = self.conf.fadc_vs_nat_intf
        listener.fadc_persistence = self.conf.fadc_vs_persistency
        listener.fadc_dev_intf = self.conf.fadc_vs_dev_intf
        ...
        if not bool(listener.admin_state_up):
