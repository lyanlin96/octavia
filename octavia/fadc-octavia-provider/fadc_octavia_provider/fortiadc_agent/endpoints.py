...
from fadc_octavia_provider.fortiadc_agent import controller_worker
...
class Endpoints(object):
    ...
    def create_load_balancer(self, context, loadbalancer,
        ...
        self.worker.create_load_balancer(loadbalancer, flavor, availability_zone)
    ...
    def failover_load_balancer(self, context, load_balancer_id):
        LOG.info('Failover load balancer \'%s\'...',
                 load_balancer_id)

    def failover_amphora(self, context, amphora_id):
        LOG.info('Failover amphora \'%s\'...',
                 amphora_id)
    ...
    def sync_state(self, context, name):
        LOG.info('sync state \'%s\'...', name)
        self.worker.sync_state(name)

    def sync_stats(self, context, name):
        LOG.info('sync stats \'%s\'...', name)
        self.worker.sync_stats(name)
