...
class MemberFlows(object):
    ...
    def get_create_member_flow(self):
        ...
        create_member_flow.add(fadc_lifecycle_tasks.MemberToErrorOnRevertTask(
            requires=[constants.MEMBER,
                      constants.LISTENERS,
                      constants.LOADBALANCER,
                      constants.POOL_ID]))
        create_member_flow.add(fadc_database_tasks.MarkMemberPendingCreateInDB(
            requires=constants.MEMBER))
        create_member_flow.add(fortiadc_driver_tasks.MemberCreate(
            requires=constants.MEMBER
        ))

        create_member_flow.add(fadc_database_tasks.MarkMemberActiveInDB(
            requires=constants.MEMBER))
        create_member_flow.add(fadc_database_tasks.MarkPoolActiveInDB(
            requires=constants.POOL_ID))
        create_member_flow.add(fadc_database_tasks.
    ...
    def get_delete_member_flow(self):
        ...
        delete_member_flow.add(fadc_lifecycle_tasks.MemberToErrorOnRevertTask(
            requires=[constants.MEMBER,
                      constants.LISTENERS,
                      constants.LOADBALANCER,
                      constants.POOL_ID]))
        delete_member_flow.add(fadc_database_tasks.MarkMemberPendingDeleteInDB(
            requires=constants.MEMBER))
        delete_member_flow.add(fortiadc_driver_tasks.MemberDelete(
            requires=constants.MEMBER))
        delete_member_flow.add(fadc_database_tasks.DeleteMemberInDB(
            requires=constants.MEMBER))
        delete_member_flow.add(fadc_database_tasks.DecrementMemberQuota(
            requires=constants.PROJECT_ID))
        delete_member_flow.add(fadc_database_tasks.MarkPoolActiveInDB(
            requires=constants.POOL_ID))
        delete_member_flow.add(fadc_database_tasks.
                               MarkLBAndListenersActiveInDB(
                                   requires=(constants.LISTENERS,
                                             constants.LOADBALANCER_ID)))
    ...
    def get_update_member_flow(self):
        ...
        update_member_flow.add(fadc_lifecycle_tasks.MemberToErrorOnRevertTask(
            requires=[constants.MEMBER,
                      constants.LISTENERS,
                      constants.LOADBALANCER,
                      constants.POOL_ID]))
        update_member_flow.add(fadc_database_tasks.MarkMemberPendingUpdateInDB(
            requires=constants.MEMBER))
        update_member_flow.add(fortiadc_driver_tasks.MemberUpdate(
            requires=constants.MEMBER))
        update_member_flow.add(fadc_database_tasks.UpdateMemberInDB(
            requires=[constants.MEMBER, constants.UPDATE_DICT]))
        update_member_flow.add(fadc_database_tasks.MarkMemberActiveInDB(
            requires=constants.MEMBER))
        update_member_flow.add(fadc_database_tasks.MarkPoolActiveInDB(
            requires=constants.POOL_ID))
        update_member_flow.add(fadc_database_tasks.
                               MarkLBAndListenersActiveInDB(
                                   requires=(constants.LISTENERS,
                                             constants.LOADBALANCER_ID)))
    ...
    def get_batch_update_members_flow(self, old_members, new_members,
        ...
        for m in new_members:
            unordered_members_active_flow.add(
                fadc_database_tasks.MarkMemberActiveInDB(
                    inject={constants.MEMBER: m},
                    name='{flow}-{id}'.format(
                        id=m[constants.MEMBER_ID],
        ...
        batch_update_members_flow.add(fadc_database_tasks.MarkPoolActiveInDB(
            requires=constants.POOL_ID))
        batch_update_members_flow.add(
            fadc_database_tasks.MarkLBAndListenersActiveInDB(
